...
if('geolocation' in navigator) {
	...
	AddLink(position.coords.latitude, position.coords.longitude);
});
} else {
	alert("Unable to use Geolocation")
}

function AddLink(la,lo) {
	var findme = document.getElementById('findme');
	// innerHTML API
	findme.innerHTML = '<a href = "https://www.openstreetmap.org/#map=18/'+la+'/'+lo+'">Open Location on Openmap</a>';
}